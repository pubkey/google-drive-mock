...
describe('Advanced Drive Features', () => {
    ...
    let req: (method: string, endpoint: string, body?: any) => Promise<any>;

    beforeAll(async () => {
        config = await getTestConfig();
        req = async (method: string, endpoint: string, body?: any) => {
    ...
    it('should support changes feed (startPageToken and listing changes)', async () => {
        ...
        const startToken = tokenRes.body.startPageToken;
        ...
        const changes = changesRes.body.changes;
        const found = changes.find((c: any) => c.fileId === fileId);
        ...
        const changes2 = changesRes2.body.changes;
        const deletion = changes2.find((c: any) => c.fileId === fileId && c.removed === true);
    ...
    it('should support advanced query operators (contains, in parents)', async () => {
        ...
        const foundParent = searchParents.body.files.find((f: any) => f.id === fileId);
        ...
        const foundContains = searchContains.body.files.find((f: any) => f.id === fileId);
        ...
        const foundTime = searchTime.body.files.find((f: any) => f.id === fileId);
    ...
    it('should support ordering', async () => {
        ...
        const files = sortRes.body.files;
        // Should be C, B, A
        expect(files.length).toBeGreaterThanOrEqual(3);
        const relevant = files.filter((f: any) => f.name.includes(prefix));
        ...
        const filesTime = sortTimeRes.body.files;
        const relevantTime = filesTime.filter((f: any) => f.name.includes(prefix));
